...
export function authorImagePrompt(input: ImagePromptInput): ImagePromptResult {
  const seedBase = `${input.brand.slug}|${input.idea.id}|${input.product || ""}|${input.formatSlug}|v${input.variantIndex}`;
  ...
  const lines = [
    ...
    `Brand reference: ${input.brand.name}. Subject consistency with brand archetype.`,
